...
function reset() {
  ...
  landuse.forEach(function(q, id) {
    ...
    // Increase the probability of adding colored blocks
    if (PRNG.random() < 0.7) {  // Changed from 0.1 to 0.7
      ...
      ctx.strokeStyle = "#FFF";
      // Use only the original color options with even distribution
      var colorOptions = ["#F65558", "#18BB6F", "#1E7FF7"];
      var colorIndex = Math.floor(id % 3);  // Ensure even distribution
      ctx.fillStyle = ctx.shadowColor = colorOptions[colorIndex];

      // Remove size-based color override to maintain even distribution
      // if (q.area > Math.pow(h / (20 + PRNG.random() * 10), 2)) {
      //   ctx.fillStyle = ctx.shadowColor = "#1E7FF7";
      // }
  ...
  landuse.forEach(function(q, id) {
    ...
    if (id % 3 == 0) {
      ...
      for (var i = 0; i < 10; i += 1) {
        ...
        ctx.shadowColor = ctx.strokeStyle = ctx.fillStyle = "#fff";
